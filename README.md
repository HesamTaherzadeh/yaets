...
2. Run the `gantt.py` script:
    ```bash
    gantt.py trace_output.log --max_traces 100
    ```
    or altenativelly
     ```bash
...
1. Run the `histogram.py` script, specifying the function name you want to analyze:
    ```bash
    histogram.py trace_output.log --function example_function --bins 50
    ```
    or altenativelly
     ```bash
...
Since the ‍‍`histogram.py` and `gantt.py` scripts have been installed in /usr/local/bin, they can be called globally from anywhere in your system, which means any executable placed in this directory can be accessed without needing to specify the full path.
